...
_start:
    ...
    call parse_intro                                #function that shifts the number from memory view to ascii number equivalent. As a result, it will increase it's size from 8 bytes to 16 bytes
                                                    #binary mask to cut out biggest byte in the reverse routine
    call time_to_print                              #call of print function

    mov $1, %rax
    mov $1, %rdi
    lea nextline(%rip), %rsi
    mov $1, %rdx
    syscall
