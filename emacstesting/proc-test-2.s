...
time_to_print:
    ...
    mov $17, %rdx                                   #setting length of 16 bits (2 per 1 number) and 1 bit for newline \n char
...
_start:
    ...
    mov var1(%rip), %r8                             #the number to print
    mov $16, %rcx                                   #the counter

    call optimizing                                 #function to avoi additional steps in case if number will be 4 or less bytes
    ...
    call parse_start                                #function that shifts the number from memory view to ascii number equivalent. As a result, it will increase it's size from 8 bytes to 16 bytes
                                                    #binary mask to cut out biggest byte in the reverse routine
    mov $0b1111111100000000000000000000000000000000000000000000000000000000, %r9
    call time_to_print                              #call of print function
    mov $60, %rax                                   #moving 60 to %rax for exit syscall
    xor %rdi, %rdi                                  #setting %rdi to 0 for exit syscall
